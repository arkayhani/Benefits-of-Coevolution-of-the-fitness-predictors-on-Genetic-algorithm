#this code contains three classes of gentic algorithms, individauls , population
# and predictors to change the fitness function go to the ORIGINALFUNCTION
import math
import random
...
import urllib
mutation=0.1
#maximum number of operation in each fitness model
maxind=35
#number of terminals - 1input and 3 constants[1,2,3] (Scmidt used a evolcing constant instead)
input_num=4
#number of registers
class_num=4
...
#population of the fittness models
class population:
    def copy( self ):
        self.hpop=[]
        for x in range(0, self.size()):
    ...
    def __str__( self ):
        ...
        for x in range(0,self.size()):
            k+=str(self.item[x])
        return k
#class of nessential function for GP
class genteics:
    @classmethod
    ...
    # fitness functions for different sampling strategies
    @classmethod
    def fit3(self,item1):
       if(item1.fit!=200):
...
#provides the statistics of the evolution
    @classmethod
 ...
 #mutates each individual by mutaterate        
    @classmethod
    def mut(self,item1):
        hcross2=item1.copy()
    ...
    def sort(self,inds):
        for j in range(0,len(inds)-1):
            ...
            if(flgsort==0):
                break
  #tournoment selection algorithm              
    @classmethod
    def tour(self):
       pop=h1
       self.hselect4=[]
...
class decoder:
   ...
   def binary(self,inp):
       ...
       return self.res
       #decoding algorithms for each sampling algorithm
   @classmethod
   def calc(self,ind):
       self.meenlist=[]
       self.reglist=[]
   ...
   def calc2(self,ind):
       ...
       for tt in arraytestin:
           ...
           self.reglist.append(self.reg)
       return self.reglist,self.meenlist 
  # the true fitness function    
def originalfit(x):
    #print(x)    
    return math.exp(math.fabs(x))*math.sin(math.degrees(x))
    #calculates the fitness of predictors
...
def ML(inp):
    ...
    #128 initial 
    h1=population(128)
    # equal to 10^7 point evaluation(every generations equals to 16.85(16 from...
    # the 2 evalution in each generation on 8 predictor samples and 0.85 comes from the 340 pint evoalution of the ...
    # fitness predictors in each 400 generations which provides the 0.05 ratio of the predictors evolution ))
    generations=2373000
...
#initiates 5 predictors
    ...
    for i in range(0,generations):
        ...
        if(i/testpoint==int(i/testpoint)):
            ...
            testpoint+=1000.0
            #trainesrs training cycle every 10 predictor cycle
            if(cycle<0):
                cycle=10
                test1=var.copy()
                genteics.fit4(test1)
...
# this function can be used when we add a GA to use the best model optimization of a problem           
...
#prints the results per 16850 point evaluation
xs=np.arange(0,len(printarray)*16850,16850)

plt.plot(xs,printarray,'-r')
    
f = open('/home/arash/GApro/coevolvesample.txt', 'w')

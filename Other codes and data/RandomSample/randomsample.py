...
def ML(inp):
    ...
    arrayin=[]
    arrayout=[]
    listsamp=random.sample(range(0,len(inp)),8)
    for inx11 in range(0,8):
   
       arrayin.append(arraytestin[listsamp[inx11]])
       arrayout.append(arraytestout[listsamp[inx11]])
    genteics.fitpop(h1)
    mins=[]
    temin=0
...
kl50=[]
#toalmin=[]
for inx7 in range(0,1):
    temos,genss,minha=ML(np.arange(-3,3,0.03))
    #totalmin.append(minha)

xs=np.arange(0,len(minha)*400*1000,400*1000)

plt.plot(xs,minha,'-r')
    #kl50.append(genss)
#print(MLresult([-3,0],temos))
#print(MLresult([0,2],temos))
f = open('/home/arash/GApro/fig3-8sample.txt', 'w')
for inx2 in range(0,len(minha)):
    f.write(str(minha[inx2])+'\n')  # python will convert \n to os.linesep
  # python will convert \n to os.linesep
